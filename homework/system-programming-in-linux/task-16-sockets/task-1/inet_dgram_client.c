/**
 * @file    inet_dgram_client.c
 * @author  amidalo
 * @date    01.06.26
 * @brief   AF_INET SOCK_DGRAM клиент для сетевого взаимодействия по UDP.
 * 
 * @details Программа реализует клиент семейства AF_INET (IPv4) с типом SOCK_DGRAM (UDP).
 * Клиент обеспечивает датаграммное (ненадежное, без установки соединения)
 * взаимодействие через сеть. Клиент подключается к серверу на порт 7777
 * локального хоста, отправляет сообщение, получает ответ и завершает работу.
 */
...
/**
 * @brief Точка входа AF_INET SOCK_DGRAM клиента.
 * 
 * @details Алгоритм AF_INET SOCK_DGRAM клиента:
...
int main(void)
{
    ...
    // 5.
    ssize_t nbytes_recv = recv(fd, (void*)buffer_recv, sizeof(buffer_recv), 0);
    if (nbytes_recv == -1)
    {
        perror("recv client");
        close(fd);
    ...
    printf("AF_INET SOCK_DGRAM клиент получил: %s\n", buffer_recv);
